...
public abstract class ConnectedSpHANode extends BaseSpHANode implements MqttCallbackExtended {
	...
	protected void publishNodeBirth(SparkplugBPayload payload) {
		...
		synchronized (seqLock) {
			...
			resetSeqNum();
			executor.execute(new MQTTPublisher(client, NAMESPACE + "/" + groupId + "/NBIRTH/" + edgeNode, payload, 0, false,
					USING_COMPRESSION));
		}
	...
	protected void publishNodeDeath(SparkplugBPayload payload) {
		...
		synchronized (seqLock) {
			...
			resetSeqNum();
			executor.execute(
					new MQTTPublisher(client, NAMESPACE + "/" + groupId + "/NDEATH/" + edgeNode, payload, 0, false, false));
		}
